...
```{r}
data_indirect <- read.csv("../data/batch1/results_indirect.csv")
data_direct <- read.csv("../data/batch1/results_direct.csv")

data <- rbind(data_indirect, data_direct)
...
data_indirect_localization <- read.csv("../../../data+analysis/05-stable-speakerRelevance/data_indirect_Localization.csv")
data_indirect_localization$id <- as.factor(data_indirect_localization$id)
data_indirect_localization %>% mutate(scienceTeam = ifelse(informationSource == "indirect", "Localization", "Cultivation")) -> data_indirect_localization
data %>% mutate(scienceTeam = ifelse(informationSource == "indirect", "Cultivation", "Localization")) -> data
data_combined <- rbind(data, data_indirect_localization)
write.csv(data_combined, file = "../../../data+analysis/05-stable-speakerRelevance/data_full.csv", row.names = FALSE)
```
...
# only data from the 'direct-source' condition
shapiro.test(data |> filter(informationSource == "direct") |> pull(update))
# only data from the 'indirect-source' condition
